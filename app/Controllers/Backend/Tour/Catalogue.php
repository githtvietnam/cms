...
class Catalogue extends BaseController{
	...
	public function index($page = 1){
		...
		$flag = $this->authentication->check_permission([
			'routes' => 'backend/tour/catalogue/index'
		]);
		if($flag == false){
		...
		if($config['total_rows'] > 0){
			$config = pagination_config_bt(['url' => 'backend/tour/catalogue/index','perpage' => $perpage], $config);
			...
			$this->data['tourCatalogueList'] = $this->AutoloadModel->_get_where([
			...
			// pre($this->data['tourCatalogueList']);
		}

		$this->data['template'] = 'backend/tour/catalogue/index';
	...
	public function create(){
			...
			if ($this->validate($validate['validate'], $validate['errorValidate'])){
		 		...
		 		if($resultid > 0){
					...
					$this->nestedsetbie->Action();
		 			$session->setFlashdata('message-success', 'Tạo Nhóm Chuyến du lịch Thành Công! Hãy tạo danh mục tiếp theo.');
 					return redirect()->to(BASE_URL.'backend/tour/catalogue/create');
		 		}
	        }else{
		...
		$this->data['template'] = 'backend/tour/catalogue/create';
		return view('backend/dashboard/layout/home', $this->data);
	}
	...
	public function update($id = 0){
			...
			if ($this->validate($validate['validate'], $validate['errorValidate'])){
		 		...
		 		if($flag > 0){
					...
					$this->nestedsetbie->Action();
		 			...
		 			$session->setFlashdata('message-success', 'Cập Nhật Nhóm Chuyến du lịch Thành Công!');
 					return redirect()->to(BASE_URL.'backend/tour/catalogue/index');
		 		}
		...
		$this->data['template'] = 'backend/tour/catalogue/update';
		return view('backend/dashboard/layout/home', $this->data);
	}
	...
	public function delete($id = 0){
		...
		$this->data['template'] = 'backend/tour/catalogue/delete';
		return view('backend/dashboard/layout/home', $this->data);
	}
	...
	private function detect_language(){
		...
		if(isset($languageList) && is_array($languageList) && count($languageList)){
			foreach($languageList as $key => $val){
				$select = $select.'(SELECT COUNT(objectid) FROM tour_translate WHERE tour_translate.objectid = tb1.id AND tour_translate.module = "tour_catalogue" AND tour_translate.language = "'.$val['canonical'].'") as '.$val['canonical'].'_detect, ';
				$i++;
			}	
		}
